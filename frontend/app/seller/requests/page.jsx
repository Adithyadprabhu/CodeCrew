...
import { getOrders, updateOrderStatus, subscribeToOrders } from '@/lib/ordersStore';
...
export default function RequestsPage() {
  ...
  useEffect(() => {
    setRequests(getOrders());
    const unsub = subscribeToOrders((orders) => setRequests(orders));
    return unsub;
  }, []);
    ...
    try {
      const res = await fetch('/api/orders/accept', {
        ...
        body: JSON.stringify({
          orderId: req.id,
          status: 'accepted',
          sellerName: req.sellerName || 'EcoCycle Seller',
          material: req.material,
          buyerId: req.buyerId || 'buyer123',
        }),
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || 'Failed');

      // Update local orders store (cross-tab sync via BroadcastChannel)
      updateOrderStatus(req.id, 'accepted');
      setRequests(prev => prev.map(r => r.id === req.id ? { ...r, status: 'accepted' } : r));

      // 🔔 Add notification (buyer sees it in real-time)
      addNotification({
        orderId: req.id,
        buyerId: req.buyerId || 'buyer123',
        message: data.notification?.message || `Your order for ${req.material} has been accepted`,
        type: 'order',
        status: 'accepted',
        sellerName: req.sellerName || 'EcoCycle Seller',
        material: req.material,
  ...
  const handleReject = async (req) => {
    ...
    try {
      ...
      updateOrderStatus(req.id, 'rejected');
      ...
      addNotification({
        orderId: req.id,
        buyerId: req.buyerId || 'buyer123',
        message: data.notification?.message || `Your order for ${req.material} was declined`,
        type: 'order',
        status: 'rejected',
        sellerName: req.sellerName || 'EcoCycle Seller',
        material: req.material,
