...
import { getOrders, subscribeToOrders } from '@/lib/ordersStore';
...
export default function MyOrdersPage() {
  ...
  const refresh = useCallback(() => {
    // Show buyer123's orders for demo
    const all = getOrders().filter(o => o.buyerId === 'buyer123' || true);
    setOrders(all);
  }, []);

  useEffect(() => {
    refresh();
    const unsub = subscribeToOrders(() => refresh());
