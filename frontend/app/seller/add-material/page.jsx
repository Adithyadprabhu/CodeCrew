...
import { addListing } from '@/lib/listingsStore';
          ...
          <div className="xl:col-span-5 space-y-6">
            ...
            {results && (
              <div className="fade-up bg-white rounded-2xl p-6 shadow-sm border border-surface-container">
                ...
                <p className="text-xs text-secondary mb-4">Analyzed {images.length} image{images.length > 1 ? 's' : ''} · Click a result to select it</p>
                  ...
                  {results.map((r, i) => (
                    <div key={r.material}
                      ...
                      className={`p-4 rounded-xl border-2 cursor-pointer transition-all
                        ${primary?.material === r.material ? 'border-primary bg-primary-fixed/10' : 'border-surface-container hover:border-primary/40'}`}
                    >
                      <div className="flex items-center justify-between mb-2">
                        <div className="flex items-center gap-2 flex-wrap">
                          {i === 0 && <span className="text-[10px] font-bold bg-primary text-white px-2 py-0.5 rounded-full uppercase">Best Match</span>}
                          <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${r.tag}`}>{r.material}</span>
                        </div>
                      ...
                      <div className="flex items-center justify-between mt-2">
                        <span className="text-xs text-secondary">Suggested: <strong className="text-primary">{r.priceRange}/kg</strong></span>
                        <span className="text-xs text-outline">{r.grade}</span>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <div className="bg-white rounded-2xl p-6 md:p-8 shadow-sm border border-surface-container sticky top-24">
              ...
              <form onSubmit={handleSubmit} className="space-y-5" noValidate>
                ...
                <div className="flex gap-4 pt-4 border-t border-surface-container">
                  ...
                  <button type="submit"
                    className="flex-[2] py-4 bg-primary text-white font-bold rounded-xl shadow-lg shadow-primary/20 hover:opacity-90 active:scale-95 transition-all flex items-center justify-center gap-2">
